...
//		Meta_feature_building.Alpha_beta_type alpha_style = Meta_feature_building.Alpha_beta_type.Absolute;
		...
		for (HouseData house : houses) {
			...
			FileWriter stats_writer = null;
			try {
				writer = new PrintWriter(house.houseName + "-alphabeta" + suffix + ".csv", "UTF-8");
		...
		try {
			...
			for (int j = 0; j < stats_avg[0].length; j++) {
				...
				for (int k = 0; k < nr_stats; k++) {
					switch (k) {
					case 0:
						stats_writer.write("& Nr clusters");
						break;
					case 1:
						stats_writer.write("& Nr nu-clusters");
						break;
					case 2:
					...
					for (int i = 0; i < stats_freq.length; i++) {
						...
						case 2:
							...
							break;
						default:
							stats_writer.write(Integer.toString(stats_freq[i][j][k]));
							break;
						}
					}
